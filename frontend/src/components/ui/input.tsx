...
const InputVariants = cva(
  `h-[21px] bg-black w-full text-[14px] font-medium focus:outline-none`,
  {
    variants: {
      variant: {
        default: "border-none",
        second: "border-b py-[20px] border-[#fff]",
        third:
          "mb-2 mr-2.5 h-full min-h-[44px] border-zinc-800 w-full rounded-lg border bg-zinc-950 px-4 py-3 text-white",
      },
      error: {
        true: "border-red-800",
      },
...
const Input: React.FC<InputProps> = ({
  ...
  return (
    <div className={cn("relative", containerClassName)}>
      ...
      />
      {error && (
        <p className="absolute -bottom-5 text-[10px] text-red-500">
          {error.message}
        </p>
      )}
    </div>
